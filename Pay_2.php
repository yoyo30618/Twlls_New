								...
								<div class="content">
									<div style="width: 100%;">
											...
											<div class="card-body">
												<div class="row">
													<div class="col text-center">
														...
														<span class="h3 font-weight-bold mb-0"><b>紙本期刊</b></span>
														<h4>請來信購買。學會秘書處EMAIL：<a href="mailto:contact.tlls@gmail.com" target="_blank"><font color="blue">contact.tlls@gmail.com</font></a>。</h4>
													</div>
												</div>
										...
										<?php
											...
											$sheet = $excel->getSheetByName('三十而立');
											$buttons = array();
											...
											if(count($buttons) > 0){
												...
												<div class="card-body">
													<div class="row">
														<div class="col text-center">
															<h5 class="card-title text-uppercase text-muted mb-0"></h5>
															<span class="h3 font-weight-bold mb-0"><b>《三十而立——台灣語文學會三十週年慶祝論文集》</b></span>
															<h5>提醒：<br>1. 《三十而立——台灣語文學會三十週年慶祝論文集》定價520元，免運費。<br>2. 6-10本7.5折、10本以上享團購價，請來信訂購：<a href="mailto:contact.tlls@gmail.com" target="_blank"><font color="blue">contact.tlls@gmail.com</font></a>（學會秘書處）。<br>
															3. 不同地址請分開訂單訂購。<br>4. 本系統於收到繳費後，會寄發電子收據。</h5>
														</div>
													</div>
													<p class="mt-3 mb-0 text-muted text-sm">');
										...
										<?php
											...
											$sheet = $excel->getSheetByName('閩南地區方言地圖集');
											$buttons = array();
											foreach ($sheet->getRowIterator(2, $sheet->getHighestRow()) as $row) {
												$cellIterator = $row->getCellIterator();
												$cellIterator->setIterateOnlyExistingCells(FALSE);
											...
											if(count($buttons) > 0){
												...
												<div class="card-body">
													<div class="row">
														<div class="col text-center">
															<h5 class="card-title text-uppercase text-muted mb-0"></h5>
															<span class="h3 font-weight-bold mb-0"><b>《閩南地區方言地圖集》</b></span>
															<h5>提醒：<br>1. 本書定價1600元，國內購買免郵資。<br>
																2. 不同地址請分開訂單訂購。<br>
																3. 國外購買除以上折扣以外，需外加郵費。請來信購買：<a href="mailto:contact.tlls@gmail.com" target="_blank"><font color="blue">contact.tlls@gmail.com</font></a>（學會秘書處）。<br>
																4. 本系統於收到繳費後，會寄發電子收據。</h5>
														</div>
													</div>
													<p class="mt-3 mb-0 text-muted text-sm">');
												foreach($buttons as $item)
								...
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										...
										<div class="modal-body">
											...
											<div class="form-group">
												<label>寄書地址與收件人姓名*</label><span>（請務必填寫郵遞區號）</span>
												<div class="row">
													<div class="col-md-9"><input id="BookAddress" type="text" class="form-control"></div>
												</div>
											</div>
											<div class="form-group">
												<label>購買本數*</label>
												<div class="row">
													<div class="col-md-9"><input id="BookQuantity" type="text" class="form-control"></div>
												</div>
											</div>
											<p>1. 有*標記者為必填。</p>
											<p>2. 以上欄位請務必確認填寫正確，將由系統自動發送電子收據。如有誤填情形，本會恕難提供重新開立收據，敬請見諒。</p>
...
<script>
    ...
    function send() {
        ...
        console.log(`newebpay_getParams.php?Email=${email}&name=${name}&phone=${phone}&ReceiptTitle=${ReceiptTitle}&TaxIDNumber=${TaxIDNumber}&ReceiptAddress=${ReceiptAddress}&theString=${totalString}`);
        // return;
        fetch(`newebpay_getParams.php?Email=${email}&name=${name}&phone=${phone}&ReceiptTitle=${ReceiptTitle}&TaxIDNumber=${TaxIDNumber}&ReceiptAddress=${ReceiptAddress}&theString=${totalString}`)
            .then(res => res.json())
            .then(json => {
                if (json.status == -1)
