...
<body class="AllBODY">
	...
	<div class="banner-bottom" style="background-image: url(images/Pay-banner.jpg);">
		<div class="container">
			<div class="video-bottom-grids" style="color:white;text-align:center;height:150px; display:flex; align-items:center; justify-content:center;">
				<text style="font-size:40px;">會員繳費與捐款</text>
			</div>
		</div>
	</div>
	<br>
	...
	<div class="container ">
		<div class="video-bottom-grids1">
			<div class="col-md-12 video-bottom-grid">
				<?php
					$PayListSQL="SELECT DISTINCT `Classification` FROM `payparameter` WHERE 1 AND `Category`='會員繳費及捐款' AND `IsUsed`='1' ORDER BY `OrderIndex` DESC";
					...
					while ($PayListrow = mysqli_fetch_array($PayListResult)) {
						...
						echo "		<h3 style='color:#FDFFA3'>".$PayListrow['Classification']."</h3>";
						echo "		<text style='color:white'>提醒：本系統於收到繳費後，會寄發電子收據。</text>";
						echo "	</div>";
						echo "	<div style='padding: 10px; height: calc(100% - 40px);'>";
							$PayListDetalSQL="SELECT * FROM `payparameter` WHERE 1 AND `Category`='會員繳費及捐款' AND `IsUsed`='1' AND `Classification`='".$PayListrow['Classification']."' ORDER BY `OrderIndex` DESC";
